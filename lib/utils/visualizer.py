...
class Visualizer(object):
    ...
    def visual_numpy_image_with_box(self, image, box, output_size, point=None, path= None, name='default', show=False):
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)

        cv2.rectangle(image, (int(box[0]), int(box[1])),
                              (int(box[0] + box[2]), int(box[1] + box[3])), (0,0,255), 3)
        if point is not None:
            cv2.circle(image, (int(point[0]), int(point[1])), 5, (0, 255, 0), 3)
        if path is not None:
            if not os.path.exists(path):
                os.makedirs(path)
            cv2.imwrite(os.path.join(path, '{:s}.jpg'.format(name)), image)

    def visual_numpy_image(self, image, output_size, path= None, name='default', show=False):
        image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
    ...
    def visualize_normed_image_with_box(self, normed_image, box, output_size, point=None, path=None, name='default', show=False):
        """
        box - [x1, y1, w, h]
        """
        image = self.unnorm(normed_image)
