...
# network related
from lib.models.stark import build_starks, build_starkst, build_stark_p
# forward propagation related
from lib.train.actors import STARKSActor, STARKSTActor, STARKPActor
# for import modules
import importlib
...
def run(settings):
    ...
    elif settings.script_name == "stark_p":
        net = build_stark_p(cfg)
    else:
    ...
    if settings.script_name == "stark_s" or settings.script_name == "stark_st1":
        ...
        objective = {'giou': giou_loss, 'l1': l1_loss}
        loss_weight = {'giou': cfg.TRAIN.GIOU_WEIGHT, 'l1': cfg.TRAIN.L1_WEIGHT}
        actor = STARKPActor(net=net, objective=objective, loss_weight=loss_weight, settings=settings)
