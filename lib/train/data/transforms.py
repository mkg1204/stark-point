...
class Transform:
    ...
    def __init__(self, *transforms):
        ...
        self._valid_inputs = ['image', 'coords', 'bbox', 'mask', 'att', 'gauss', 'point']
...
class TransformBase:
    ...
    def __init__(self):
        """2020.12.24 Add 'att' to valid inputs, 2021.04.15 Add 'gauss' to valid inputs, 2021.06.12 Add 'point' to valid inputs."""
        self._valid_inputs = ['image', 'coords', 'bbox', 'mask', 'att', 'gauss', 'point']
            ...
            if var is not None:
                ...
                if isinstance(var, (list, tuple)):
                    ...
                    try:
                        ...
                        outputs[var_name] = transform_func(var, input_vars['bbox'], *params)
        return outputs
    ...
    def transform_gauss(self, gauss, *rand_params):
        """2021.04.15 Added to deal with gaussian masks"""
        return gauss

    def transform_point(self, point, *rand_params):
        """2021.6.12 Added to deal with point"""
        return point
...
class ToTensor(TransformBase):
    ...
    def transform_gauss(self, gauss):
        if isinstance(gauss, np.ndarray):
            return torch.from_numpy(gauss).to(torch.float)
        elif isinstance(gauss, torch.Tensor):
            return gauss.to(torch.float)
        else:
            raise ValueError ("dtype must be np.ndarray or torch.Tensor")
...
class ToTensorAndJitter(TransformBase):
    ...
    def transform_gauss(self, gauss, brightness_factor):
        if isinstance(gauss, np.ndarray):
            return torch.from_numpy(gauss).to(torch.float)
        elif isinstance(gauss, torch.Tensor):
            return gauss.to(torch.float)
...
class RandomHorizontalFlip(TransformBase):
    ...
    def transform_gauss(self, gauss, do_flip):
        if do_flip:
            if torch.is_tensor(gauss):
                return gauss.flip((-1,))
            return np.fliplr(gauss).copy()
        return gauss
    
    def transform_point(self, point, do_flip):
        if do_flip:
            point[0] = 1 - point[0]
        return point
...
class Blur(TransformBase):
    ...
    def __init__(self, sigma, probability=0.2):
        super().__init__()
        self.probability = probability
    ...
    def transform_image(self, img, bbox, do_blur):
        if do_blur:
            if isinstance(img, torch.Tensor):
                sz = img.shape[1:]
                bbox_center = (bbox[0:2] + bbox[2:4]/2).flip(dims=[0]) * torch.tensor(sz)# [yc, xc]
                bbox_size = bbox[2:4].flip(dims=[0]) * torch.tensor(sz)# [h, w]

                blur_region_size = bbox_size * (1 + 2 * torch.rand(2))
                blur_region_center = bbox_center + blur_region_size * (torch.rand(2) - 0.5)
                blur_region_tl = torch.clamp(torch.round(blur_region_center - blur_region_size / 2), min=0).to(torch.int)
                blur_region_br = torch.round(blur_region_center + blur_region_size / 2)
                blur_region_br = torch.tensor([min(blur_region_br[0], sz[0]-1), min(blur_region_br[1], sz[1]-1)]).to(torch.int)

                blur_region = img[:, blur_region_tl[0]: blur_region_br[0]+1, blur_region_tl[1]: blur_region_br[1]+1]
                try:
                    blur_region1 = F.conv2d(blur_region.unsqueeze(1), self.filter[0], padding=(0, 0))
                    blur_region2 = F.conv2d(blur_region1, self.filter[1], padding=(0,0)).squeeze()
                    img[:, blur_region_tl[0]+self.filter_size[0]: blur_region_br[0]+1-self.filter_size[0],
                        blur_region_tl[1]+self.filter_size[1]: blur_region_br[1]+1-self.filter_size[1]] = blur_region2
                except:
                    pass
                return img
            else:
