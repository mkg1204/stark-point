...
class TrackingSampler(torch.utils.data.Dataset):
    ...
    def __init__(self, datasets, p_datasets, samples_per_epoch, max_gap, max_gap_within_clip,
                 num_search_frames, num_template_frames=1, processing=no_processing, frame_sample_mode='causal',
                 train_cls=False, pos_prob=0.5, log_file=None):
        """
        args:
            ...
            max_gap_within_clip - Maximum gap within a clip, between the first and the last frames in a clip
        ...
        self.max_gap_within_clip = max_gap_within_clip
    ...
    def __getitem__(self, index):
        #TODO train_cls具体是怎么操作的，我们的代码是否要兼容没有visible target的情况,这样更符合追踪的任务需求
        if self.train_cls:
    ...
    def getitem(self):
        ...
        """
        valid = False
        while not valid:# zikun 直到返回有效数据才停止
            # Select a dataset
            ...
            while not sample_succeed:
                ...
                #print(dataset, seq_id)
                if is_video_dataset:
                    template_frame_ids = None
                    search_frame_ids = None
                    gap_increase = 0

                    if self.frame_sample_mode == 'causal':# zikun STARK_s use casual, while STARK_st use trident_pro
                        # Sample test and train frames in a causal manner, i.e. search_frame_ids > template_frame_ids
                        while search_frame_ids is None:
                            sample_succeed = False
                            # 至少有一个是visible, 所以base_frame_id 不会是 None
                            base_frame_id = self._sample_visible_ids(visible, num_ids=1, min_id=self.num_template_frames - 1,
                                                                  max_id=len(visible) - self.num_search_frames)
                            # prev_frame_ids 是在有需要的情况下对base_frame_id前面的样本进行采样
                            ...
                            # seach_frame是在template_frame后面并且满足最大采样间隔的限制，当预定义的采样间隔内无法找到合适的样本的时，就增大采样间隔，知道找到合适的样本
                            ...
                            if search_frame_ids is None:
                                with open(self.log_file, 'a') as f:
                                    f.write('Condition failure: Invalid search frame ids, try again!\n')
                    ...
                    elif self.frame_sample_mode == "clip":
                        while search_frame_ids is None:
                            sample_succeed = False
                            if gap_increase >= 100:# if failed multiple times, try another video
                                print('Changing to another sequence!')
                                with open(self.log_file, 'a') as f:
                                    f.write('Changing to another sequence!\n')
                                break

                            # get template clip and search clip in a 'clip' manner
                            # we first sample the last template image in the template clip, and then sample the previous template images
                            last_template_id = self._sample_visible_ids(visible, num_ids=1, min_id=self.num_template_frames - 1,
                                                                    max_id=len(visible) - self.num_search_frames)
                            prev_template_ids = self._sample_visible_ids(visible, num_ids=self.num_template_frames - 1,
                                                                     min_id=last_template_id[0] - self.max_gap_within_clip - gap_increase,
                                                                     max_id=last_template_id[0])
                            if prev_template_ids is None:
                                gap_increase += 5
                                continue
                            template_frame_ids = prev_template_ids + last_template_id

                            # the search clip is sampled after the template clip in the temporal domain
                            # we first sample the first search image in the search clip, and then sample the subsequent search images
                            first_search_id = self._sample_visible_ids(visible, num_ids=1, min_id=template_frame_ids[-1] + 1,
                                                                       max_id=min(template_frame_ids[-1] + self.max_gap + gap_increase,
                                                                       len(visible)-self.num_search_frames))
                            if first_search_id is None:
                                gap_increase += 5
                                continue
                            post_search_ids = self._sample_visible_ids(visible, num_ids=self.num_search_frames - 1,
                                                                   min_id=first_search_id[0],
                                                                   max_id=first_search_id[0] + self.max_gap_within_clip + gap_increase)
                            search_frame_ids = first_search_id + post_search_ids
                            gap_increase += 5
                            sample_succeed = True
                            if search_frame_ids is None:
                                with open(self.log_file, 'a') as f:
                    ...
                    sample_succeed = True
                if not sample_succeed:
                    with open(self.log_file, 'a') as f:
                        f.write('Condition failure: Sample failed, try again!\n')
            ...
            try:
                ...
                # zikun 只有COCO数据集是有mask的, 其他的数据集的mask都是全0矩阵
                ...
                data = TensorDict({'template_images': template_frames,
                                   ...
                                   'template_visible': template_anno['visible'],# torch.uint8
                                   'template_masks': template_masks,
                                   'search_images': search_frames,
                                   'search_anno': search_anno['bbox'],
                                   'search_visible': search_anno['visible'],# torch.uint8
                ...
                print('Exception occured during sampling:', e)
                valid = False
            if not valid:
                with open(self.log_file, 'a') as f:
                    f.write('Condition failure: Retrieve invalid data, try again!\n')
    ...
    def getitem_cls(self):
        ...
        while not valid:
            ...
            try:
                ...
                print("Exception accured during sampling with cls: ", e)
                valid = False
            if not valid:
                with open(self.log_file, 'a') as f:
                    f.write('Condition failure: Retrieve invalid data, try again!\n')
    ...
    def sample_seq_from_dataset(self, dataset, is_video_dataset):
        ...
        while not enough_visible_frames:
            ...
            if not enough_visible_frames:
                print('Condition failure: Not enough visible frames, try again!')
    ...
    def get_frame_ids_trident(self, visible):
        ...
        while None in template_frame_ids_extra or len(template_frame_ids_extra) == 0:
            template_frame_ids_extra = []
            # first randomly sample two frames from a video, 对于顺序和时序间隔都没有限制, 甚至采样到同一帧都可以
            ...
            for max_gap in self.max_gap:# STARK_st的配置文件里面max_gap是一个list
                ...
                if self.frame_sample_mode == "trident_pro":
                    # zikun 这里设置 allow_invisible 应该是为了鲁棒性, 因为实际追踪的时候updated template可能是有问题的
