...
def run_training(script_name, config_name, cudnn_benchmark=True, local_rank=-1, save_dir=None, base_seed=None,
    ...
    settings.project_path = 'train/{}/{}'.format(script_name, config_name)  # train/stark_s/baseline
    if script_name_prv is not None and config_name_prv is not None:
        settings.project_path_prv = 'train/{}/{}'.format(script_name_prv, config_name_prv)  # train/None/baseline
    settings.local_rank = local_rank
    settings.save_dir = os.path.abspath(save_dir)   # /home/zikun/data/mkg/Projects/Stark
    settings.use_lmdb = use_lmdb    # 0
    prj_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), "../..")) 
    settings.cfg_file = os.path.join(prj_dir, 'experiments/%s/%s.yaml' % (script_name, config_name))    # /home/zikun/data/mkg/Projects/Stark/experiments/stark_s/baseline.yaml

    expr_module = importlib.import_module('lib.train.train_script')
    expr_func = getattr(expr_module, 'run')
    # print(['%s:%s' % item for item in settings.__dict__.items()])
    expr_func(settings)
